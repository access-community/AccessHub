...
function secondsToHuman(value) {
  const years = Math.floor(value / (86400 * 365));
  const months = Math.floor((value % (86400 * 365)) / (86400 * 30));
  const days = Math.floor(((value % (86400 * 365)) % (86400 * 30)) / 86400);
...
<template>
  ...
  <div
    class="max-w-7xl mx-auto p-12 pt-3 xl:p-0 xl:pt-12 duration-75"
  >
    ...
    <h1 class="text-white mb-5 text-2xl font-medium flex items-center flex-wrap">
      ...
      <span class="font-normal text-lg block w-full">Access propose des serveurs de jeu sur SCP:SL.</span>
    </h1>
    ...
    <div class="grid justify-between gap-4 grid-cols-3">
      ...
      >
        <div class="flex-row flex items-center gap-4">
          ...
          <span class="flex w-12 h-12 items-center justify-center text-2xl font-bold font-heading rounded-full bg-violet-100 text-violet-600">
            <font-awesome-icon
              :icon="['fas', 'server']"
            />
          </span>
    ...
    <div v-if="statistics">
      ...
      <h1 class="text-white my-5 text-2xl font-medium grid">
        Statistiques
      </h1>
